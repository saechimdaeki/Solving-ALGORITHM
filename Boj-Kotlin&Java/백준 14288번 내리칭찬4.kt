...
private lateinit var rtree:IntArray
private lateinit var rlazy:IntArray
...
fun main() {
    ...
    rtree= IntArray(size)
    rlazy= IntArray(size)
    var check=false //false 는 아래 true는 위로 합시다잉
    repeat(m) {
        ...
        when (what) {
            1 -> {
                ...
                val index2 = temp[2]
                when {
                    !check -> updateRange(tree, lazy, 1, 1, t, inr[index], outr[index], index2)
                    else -> updateRange(rtree, rlazy,1,1,t,inr[index], inr[index],index2)
                }
            }
            2 -> {
                val index = temp[1]
                bw.write("${sum(tree,lazy,1,1,t,inr[index],inr[index])+sum(rtree, rlazy,1,1,t,inr[index],outr[index])}\n")
            }
            else -> {
                check=!check
            }
